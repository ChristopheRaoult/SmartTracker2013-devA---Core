using System;

namespace SDK_SC_RfidReader.DeviceBase
{
    /// <summary>
    /// Class CRC : Contain varaible and method for compute an verify CRC in a message
    /// </summary>
    class CRC
    {
        /// <summary>
        /// Variable  static byte[] : High Word value look-up table
        /// </summary>
        static byte[] CrcLutHi = new byte[]
		{
			
		};
        /// <summary>
        /// Variable  static byte[] : Low Word value look-up table
        /// </summary>
        static byte[] CrcLutLo = new byte[]
		{
			
		};
        /// <summary>
        /// Variable  int : High Word of CRC compute
        /// </summary>
        private int ucCrcHi = 0xFF;
        /// <summary>
        /// Variable  int : Low Word of CRC compute
        /// </summary>
        private int ucCrcLo = 0xFF;

        //-------------------------------------------------------------------------------------------------
        /// <summary>
        /// Property to retrieve CRC Value
        /// </summary>
        ///<value>Message stores the CRC as big endian, so swap the result bytes.</value>
        public UInt16 crc
        {
            get
            {
                // Message stores the CRC as big endian, so swap the result bytes.
                return ((UInt16)((ucCrcLo << 8) | ucCrcHi));
            }
        }

        //-------------------------------------------------------------------------------------------------
        /// <summary>
        /// Method to reset High and Low word of CRC result.
        /// </summary>
        public void reset()
        {
            ucCrcHi = 0xFF;
            ucCrcLo = 0xFF;
        }

        //-------------------------------------------------------------------------------------------------
        /// <summary>
        /// Method to add a message in the buffer and compute CRC
        /// </summary>
        /// <param name="message">Message to add in a string format.</param>
        public void addBuffer(string message)
        {
            for (int i = 0; i < message.Length; i++)
            {
                int uLutIndex = ucCrcHi ^ message[i];
                ucCrcHi = CrcLutHi[uLutIndex] ^ ucCrcLo;
                ucCrcLo = CrcLutLo[uLutIndex];
            }
        }

        //-------------------------------------------------------------------------------------------------
        /// <summary>
        /// Method to add a message in the buffer and compute CRC
        /// </summary>
        /// <param name="buffer">Message to add in array of byte format</param>
        public void addBuffer(byte[] buffer)
        {
            foreach (byte bufferByte in buffer)
            {
                int uLutIndex = ucCrcHi ^ bufferByte;
                ucCrcHi = CrcLutHi[uLutIndex] ^ ucCrcLo;
                ucCrcLo = CrcLutLo[uLutIndex];
            }
        }

        //-------------------------------------------------------------------------------------------------
        //
        /// <summary>
        /// Method calculatePbCRC :  Calculate the CRC for the specified message.
        /// </summary>
        /// <param name="message">Message to compute</param>
        /// <returns>The CRC result in an integer</returns>
        public static UInt16 calculatePbCRC(string message)
        {
            CRC crcCalculator = new CRC();
            crcCalculator.addBuffer(message);
            return crcCalculator.crc;
        } // calculatePbCRC()
    }
}
