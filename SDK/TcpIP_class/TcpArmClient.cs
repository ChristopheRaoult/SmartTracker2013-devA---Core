...
namespace TcpIP_class
{
    ...
    public class TcpArmDevice
    {
        ...
        public string IpAdress {get { return _ipServer; }}
        public int PortServer {get { return _portServer; }}
        ...
        /// Ip address.
        /// </param>
        public TcpArmDevice(string ipAddress, int portServer)
        {
            this._ipServer = ipAddress;
            this._portServer = portServer;
            _tcpClient = new TcpArmClient(_ipServer, _portServer, this);
            ...
            if (ConnectedEvent.WaitOne(2000))
            {
                ...
                Console.WriteLine("Connected to " + _deviceType + " (" + _serialNumber + ") on " + _ipServer + ":" + _portServer);
                return;
            }
        ...
        public InventoryData GetLastInventory()
        {
            ...
            InventoryData tmpInvData = SerializationHelper.DeserializeInventory(serializedInventory);
            tmpInvData.ListTagWithChannel = getTagByDrawer();
        ...
        public void RequestScan(ScanOption[] scanOption)
        {
            if (scanOption != null)
            {
                ...
                {
                    case 0: _tcpClient.Send(RequestCode.SCAN); break;
                    case 1: _tcpClient.Send(RequestCode.SCAN , scanOption[0].ToString()); break;
                    case 2: _tcpClient.Send(RequestCode.SCAN, scanOption[0].ToString(), scanOption[1].ToString()); break;
                    case 3: _tcpClient.Send(RequestCode.SCAN, scanOption[0].ToString(), scanOption[1].ToString(), scanOption[2].ToString()); break;
                    case 4: _tcpClient.Send(RequestCode.SCAN, scanOption[0].ToString(), scanOption[1].ToString(), scanOption[2].ToString(), scanOption[3].ToString()); break;
                    default: _tcpClient.Send(RequestCode.SCAN); break;
                }
            }
        }
        ...
        /// <returns>A constant value from RewriteUIDResult.</returns>
       /* public byte RewriteUid(string oldUid, string newUid)
        {
            try
            {
                string[] packets =
                ...
                if (RequestCode.REWRITE_UID != packets[0])
                {
                    return RewriteUIDResult.ERROR;
                }

                return Byte.Parse(packets[1]);
            }
            catch (Exception e)
            {
                if (e is FormatException || e is OverflowException)
                {
                    // TODO: log exception
                    Console.WriteLine("Invalid RewriteUIDResult value.");
                }
        ...
        public Hashtable getTagByDrawer()
        {
            string[] packets = _tcpClient.SendSynchronousRequest(RequestCode.TAG_TO_DRAWER);

            if (packets == null || packets.Length < 2 || RequestCode.TAG_TO_DRAWER != packets[0])
            {
                return null;
            }

            Hashtable tagToDrawer = new Hashtable();
            int size = packets.Length - 1;
            for (int loop = 1; loop < size; loop += 2)
                tagToDrawer.Add(packets[loop].ToString(), packets[loop + 1]);
    ...
    public static class RequestCode
    {
       ...
       // public const string INVENTORY_RANGE = "inventoryrange";
        ...
        // ask to start a LED lighting operation between multiple readers
        public const string START_LIGHTING_ACROSS = "startlightingacross";
        // ask to stop a LED lighting operation between multiple readers
        public const string STOP_LIGHTING_ACROSS = "stoplightingacross";

        //ask to get tag to drawer mapping list
        public const string TAG_TO_DRAWER = "tagtodrawer";
        //ask to get tag to drawer mapping list
        public const string TAG_TO_DRAWER_BY_ID = "tagtodrawerbyid";
    }
    ...
    public static class SerializationHelper
    {
        ...
        /*public static DeviceGrant DeserializeUser(string serializedUser)
        {
            ...
            if (fingerNodes.Count > 0)
            {
                ...
                {
                    var fingerIndex = GetFingerIndexFromJavaValue(fingerNode.Attributes["index"].Value);
                    dg.user.isFingerEnrolled[(int)fingerIndex] = true;
                    userClass.strFingerprint[(int) fingerIndex] = fingerNode.InnerText;
                   
                    // TODO: Check the template (base64) is compatible with OneTouch SDK, else, convert it
                    // (SmartServer is using UareUSDK)
                }
        ...
        /*public static string SerializeUser(DeviceGrant newUser)
        {
            ...
            if (newUser.user.template != null)
            {
                ...
                for (int i = 0; i < 10; ++i)
                {
                    ...
                    string fingerIndex = GetStringFingerIndexFrom((FingerIndexValue)i);
        ...
        /// <summary>Get a FingerIndexValue value from the given string.</summary>
        /// <param name='valueFromXml'>
        /// String value from XML serialization. 
        /// See FingerIndex enum in Java SDK.
        /// </param>
        /// <returns>Corresponding FingerIndexValue (UnknownFinger if value is invalid).</returns>
       /* private static FingerIndexValue GetFingerIndexFromJavaValue(string valueFromXml)
        {
            switch (valueFromXml)
            {                   

                case "0":
                    return FingerIndexValue.Left_Little;

                case "1":
                    return FingerIndexValue.Left_Ring;

                case "2":
                    return FingerIndexValue.Left_Middle;

                case "3":
                    return FingerIndexValue.Left_Index;

                case "4":
                    return FingerIndexValue.Left_Thumb;

                case "5":
                    return FingerIndexValue.Right_Thumb;

                case "6":
                    return FingerIndexValue.Right_Index;

                case "7":
                    return FingerIndexValue.Right_Middle;

                case "8":
                    return FingerIndexValue.Right_Ring;

                case "9":
                    return FingerIndexValue.Right_Little;

                default:
                    return FingerIndexValue.Unknown_Finger;
            }
        }*/
        ...
        /// </summary>
        /// <param name='fingerIndex'>C# SDK "FingerIndexValue" enum value</param>
        /// <returns>The corresponding value to be used with SmartServer</returns>
      /*  public static int GetDpfpFingerIndex(FingerIndexValue fingerIndex)
        {
            int oldIndex = (int)fingerIndex;
            ...
            // left hand is 5->9 (thumb to pinky) in C#, 0->4 in Java (pinky to thumb)
            return Math.Abs(oldIndex - 9);
        }*/

        /// <summary>
        /// Allow converting a FingerIndexValue enum value to a String that SmartServer understands.
        /// </summary>
        /// <param name='fingerIndexValue'>Value to be "converted".</param>
        /// <returns>A FingerIndex enum value name (see FingerIndex enum in Java SDK).</returns>
      /*  private static string GetStringFingerIndexFrom(FingerIndexValue fingerIndexValue)
        {
            switch (fingerIndexValue)
            {
                case FingerIndexValue.Left_Little:
                    return "0";

                case FingerIndexValue.Left_Ring:
                    return "1";

                case FingerIndexValue.Left_Middle:
                    return "2";

                case FingerIndexValue.Left_Index:
                    return "3";

                case FingerIndexValue.Left_Thumb:
                    return "4";

                case FingerIndexValue.Right_Thumb:
                    return "5";

                case FingerIndexValue.Right_Index:
                    return "6";

                case FingerIndexValue.Right_Middle:
                    return "7";

                case FingerIndexValue.Right_Ring:
                    return "8";

                case FingerIndexValue.Right_Little:
                    return "9";
