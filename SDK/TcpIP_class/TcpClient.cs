...
namespace TcpIP_class
{
    public class TcpIpClient
    {
        ...
        public void CreateArmDevice(string ip, int port)
        {
             try
            {
                ...
                _tcpArmDevice = new TcpArmDevice(ip , port);
                ...
                try
                {
                    ...
                    if (GetData(tcpclnt, out response) == 1)
                    {
                        ...
                        // add each optional parameters (= tag ID(s) not found / not lighted) in a new list (that the client will use after this call)
                        {
                            ...
                            {
                                lstTmp.Add(responseFragments[i].Trim());
                            }
                            tagsToLight.RemoveAll(uid => !lstTmp.Contains(uid));
                        }
                        else
                             tagsToLight.Clear();
        ...
        public RetCode UnlockDoor(string strIP, int port)
        {
            if (_cpuKind == CpuKind.Unknown)
            {
                GetDeviceKind(strIP, port);
        ...
        public RetCode lockDoor(string strIP, int port)
        {
            if (_cpuKind == CpuKind.Unknown)
            {
                GetDeviceKind(strIP, port);
